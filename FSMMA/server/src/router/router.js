...
router.get('/admin-view-student/:id', (req, res, next) => {
  let selectQuery = 'SELECT first_name, last_name, email, phone, dob, address, city, state, postal_code, account_created_date, waiver_signed_date, session_credits FROM Student_Account WHERE student_id = ?';
...
router.get('/student-view-schedule', (req, res, next) => {
  let selectQuery = 'SELECT S.session_id, S.date, S.time, I.first_name AS i_first_name, I.last_name AS i_last_name,' +
                    'I.instructor_id, SST.session_status_desc, SA.student_id ' +
                    'FROM Session S ' +
                    'JOIN Instructor I ON S.instructor_id = I.instructor_id ' +
                    'JOIN Session_Student SS ON S.session_id = SS.session_id '+
                    'JOIN Student_Account SA ON SA.student_id = SS.student_id '+
                    'JOIN Session_status SST ON SST.session_status_id = S.session_status_id '+
                    'WHERE S.date BETWEEN (DATE_ADD(NOW(), INTERVAL -30 DAY)) AND (DATE_ADD(NOW(), INTERVAL 45 DAY));';
  let query = mysql.format(selectQuery);
  pool.query(query, (err, result) => {
    if (err){
      console.error(err);
