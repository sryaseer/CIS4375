      ...
      <div id="form-div">
        <form @submit.prevent="submit" id="instructorForm">
          ...
          <validation-provider v-slot="{ errors }" name="title" rules="required|email">
            <v-text-field v-model="title" :error-messages="errors" label="Title" required outlined> </v-text-field>
          </validation-provider>

          <validation-provider v-slot="{ errors }" name="rate" rules="required|number">
            <v-text-field type="number" v-model="rate" :error-messages="errors" label="Rate per session" required outlined> </v-text-field>
          </validation-provider>

          <v-select :items="locations" v-model="location" filled label="Location" :disabled="disable"></v-select>
          ...
          <validation-provider v-slot="{ errors }" name="Phone Number" :rules="{ required: true, digits: 10 }">
            <v-text-field v-model="phoneNumber" :counter="10" :error-messages="errors" label="Phone Number" required outlined> </v-text-field>
          </validation-provider>
...
export default {
  ...
  mounted() {
    document.title = "FS MMA Create a Instructor";
  },
};
</script>
